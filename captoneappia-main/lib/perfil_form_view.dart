...
class _PerfilFormViewState extends State<PerfilFormView> {
  ...
  void _guardarPerfil() async {
    if (_formKey.currentState!.validate()) {
      ...
      final double nuevoPeso = double.tryParse(_pesoController.text.trim()) ?? 0.0;
      ...
      try {
        ...
        await DataService.registrarPeso(idUsuarioPrueba, nuevoPeso);

        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("✅ ¡Perfil y peso histórico guardados!"), backgroundColor: Colors.green),
        );
        Navigator.pop(context, true); 
      } catch (e) {
          print("DEBUG: ERROR COMPLETO: $e"); // ESTO ES LO MÁS IMPORTANTE
          if (!mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("❌ Error técnico: $e"), backgroundColor: Colors.red),
          );
      } finally {
